...
test_that("r,s,u,x kernel-based learners product the correct output format and predict well when the problem is easy", {
 ...
 easy_sim_data = data_simulation(n) # draw a sample
  ...
  s1.fit = skern(easy_sim_data$x, easy_sim_data$w, easy_sim_data$y+1)
  s1.pred = predict(s1.fit)
  invariate_add_tests(s.pred, s1.pred)

  s2.fit = skern(easy_sim_data$x, easy_sim_data$w, easy_sim_data$y*2)
  ...
  t1.fit = tkern(easy_sim_data$x, easy_sim_data$w, easy_sim_data$y+1)
  t1.pred = predict(t1.fit)
  invariate_add_tests(t.pred, t1.pred)

  t2.fit = tkern(easy_sim_data$x, easy_sim_data$w, easy_sim_data$y*2)
  ...
  u1.fit = ukern(easy_sim_data$x, easy_sim_data$w, easy_sim_data$y+1)
  u1.pred = predict(u1.fit)
  invariate_add_tests(u.pred, u1.pred)

  u2.fit = ukern(easy_sim_data$x, easy_sim_data$w, easy_sim_data$y*2)
  ...
  x1.fit = xkern(easy_sim_data$x, easy_sim_data$w, easy_sim_data$y+1)
  x1.pred = predict(x1.fit)
  invariate_add_tests(x.pred, x1.pred)

  x2.fit = xkern(easy_sim_data$x, easy_sim_data$w, easy_sim_data$y*2)
